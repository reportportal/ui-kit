...
import styles from './adaptiveTagList.module.scss';
    ...
    return (
      <div className={cx({ 'tag-list__item--button-wrapper': isShowAllView })}>
        <Button
          ...
          variant="text"
        >
          {isShowAllView && defaultVisibleLines ? hideAllText : showLessText}
        </Button>
      </div>
    );
  }, [handleButtonClick, isShowAllView, defaultVisibleLines, hideAllText, showLessText]);
    ...
    return (
      <div className={cx('tag-list-wrapper')}>
        <div className={cx('tag-list', 'tag-list--no-tags', 'tag-list--full-width')}>
          <div className={cx('no-tags-message')}>{noTagsMessage}</div>
        </div>
      </div>
    );
  }
